...
tools/simg_tools_linux/img2simg --version
unzip *.zip
...
sparseSizeCheck() {
    ...
    tools/simg_tools_linux/img2simg system.img temp.img
    rm system.img
    mv temp.img system.img
    printf "Sparsing system_ext...\n"
    tools/simg_tools_linux/img2simg system_ext.img temp.img
    rm system_ext.img
    mv temp.img system_ext.img
    printf "Sparsing product...\n"
    tools/simg_tools_linux/img2simg product.img temp.img
    rm product.img
    mv temp.img product.img
    printf "Sparsing vendor...\n"
    tools/simg_tools_linux/img2simg vendor.img temp.img
    rm vendor.img
...
if [ -f vendor.img ]; then
    ...
    tools/fastboot_linux $* snapshot-update cancel
    ...
    if [ -f odm.img ]; then
        ...
        tools/fastboot_linux $* flash odm odm.img
    fi
    if [ -f vbmeta.img ]; then
        tools/fastboot_linux $* flash vbmeta vbmeta.img
        tools/fastboot_linux $* flash vbmeta_system vbmeta_system.img
    fi
    ...
    if [ -f dtbo.img ]; then
        tools/fastboot_linux $* flash dtbo dtbo.img
    fi
